...
namespace ArchipelagoDiscordClientLegacy.Commands
{
    public static class ConnectionCommands
    {
        ...
        public static async Task HandleReConnectCommand(SocketSlashCommand command, DiscordBot discordBot)
        {
            var Data = command.GetCommandData();
            if (Data.socketTextChannel is null)
            {
        ...
        public static async Task ConnectToAPServer(SocketSlashCommand command, DiscordBot discordBot, CommandData.CommandDataModel Data, Sessions.ArchipelagoConnectionInfo ConnectionData)
        {
